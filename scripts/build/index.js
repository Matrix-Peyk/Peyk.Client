...
try {
    ...
    console.debug('building the final web app with "peyk-client:latest" tag')
    $.exec(`
        docker                  \
        build                   \
        --tag peyk-client   \
    ...
    console.debug('building the solution with "Release" configuration and "peyk-client:solution" tag')
    $.exec(`
        docker                              \
        build                               \
        --tag peyk-client:solution      \
        --no-cache                          \
    ...
    if (docker_options) {
        ...
        docker_deployment.deploy(
            'peyk-client:solution',
            'peyk/client:unstable-solution',
            docker_options.user,
            docker_options.pass
        )
